...
public:
    ...
    {
        bool ok = false;
        QString value;     // ciphertext payload (encrypt) or plaintext (decrypt)
        QString error;     // error details if ok == false
    };

    // Encrypt plaintext with raw 32-byte key -> Base64URL payload
    static Result encryptToBase64Url(const QString& plaintext, const QByteArray& key32);

    // Decrypt Base64URL payload with raw 32-byte key -> plaintext
